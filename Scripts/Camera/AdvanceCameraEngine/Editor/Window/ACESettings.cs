...
{
    [InitializeOnLoad]
    internal sealed class ACESettings
    {
        public static class ACECoreSettings
        {
                ...
                if (sCinemachineLogoTexture == null)
                    sCinemachineLogoTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(
                        ScriptableObjectUtility.CinemachineRealativeInstallPath
                            + "/Editor/Resources/cm_logo_sm.png");
                if (sCinemachineLogoTexture != null)
       ...
       internal static Texture2D CinemachineHeader
            ...
            get
            {
                if (sCinemachineHeader == null)
                    sCinemachineHeader = AssetDatabase.LoadAssetAtPath<Texture2D>(
                        ScriptableObjectUtility.CinemachineRealativeInstallPath
                            + "/Editor/Resources/cinemachine_header.tif");
                ;
                if (sCinemachineHeader != null)
        ...
        {
            var provider = new SettingsProvider("Preferences/Ace", SettingsScope.User, SettingsProvider.GetSearchKeywordsFromGUIContentProperties<Styles>());
        ...
        private static void OnGUI()
        {
            ...
            if (ShowCoreSettings)
            {
                ...
                EditorGUI.BeginChangeCheck();
                Color newActiveGizmoColour = EditorGUILayout.ColorField(Styles.sCoreActiveGizmosColour, ACECoreSettings.ActiveGizmoColour);

                if (EditorGUI.EndChangeCheck())
                {
                    ACECoreSettings.ActiveGizmoColour = newActiveGizmoColour;
                    UnityEditorInternal.InternalEditorUtility.RepaintAllViews();
                }

                if (GUILayout.Button("Reset"))
                {
                    ACECoreSettings.ActiveGizmoColour = ACECoreSettings.kDefaultActiveColour;
                }
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                EditorGUI.BeginChangeCheck();
                Color newInactiveGizmoColour = EditorGUILayout.ColorField(Styles.sCoreInactiveGizmosColour, ACECoreSettings.InactiveGizmoColour);

                if (EditorGUI.EndChangeCheck())
                {
                    ACECoreSettings.InactiveGizmoColour = newInactiveGizmoColour;
                    UnityEditorInternal.InternalEditorUtility.RepaintAllViews();
                }

                if (GUILayout.Button("Reset"))
                {
                    ACECoreSettings.InactiveGizmoColour = ACECoreSettings.kDefaultInactiveColour;
                }
                EditorGUILayout.EndHorizontal();
                EditorGUI.indentLevel--;
            }
        ...
        private static void OnHierarchyGUI(int instanceID, Rect selectionRect)
        {
            ...
            if (instance.GetComponent<ACEBrain>() != null)
            {
