...
namespace Ace.Editor
{
    ...
    internal class CinemachineFramingTransposerEditor : BaseEditor<FramingTransposer>
    {
        ...
        protected override void GetExcludedPropertiesInInspector(List<string> excluded)
        {
            ...
            IACETargetGroup group = Target.AbstractFollowTargetGroup;
            ...
            {
                ACEBrain brain = ACECore.Instance.FindPotentialTargetBrain(Target.VirtualCamera);
        ...
        protected virtual void OnEnable()
        {
            ...
            ACEDebug.OnGUIHandlers -= OnGUI;
            ACEDebug.OnGUIHandlers += OnGUI;
            if (ACESettings.ACECoreSettings.ShowInGameGuides)
        ...
        protected virtual void OnGUI()
        {
            ...
            if (!Target.IsValid || !ACESettings.ACECoreSettings.ShowInGameGuides)
                return;

            ACEBrain brain = ACECore.Instance.FindPotentialTargetBrain(Target.VirtualCamera);
            if (brain == null || (brain.OutputCamera.activeTexture != null && ACECore.Instance.BrainCount > 1))
                return;

            bool isLive = ACECore.Instance.IsLive(Target.VirtualCamera);
            ...
            if (Target.FollowTarget != null && isLive)
            {
                ...
                if (targetScreenPosition.z > 0)
                {
                    ...
                    GUI.color = ACESettings.ComposerSettings.TargetColour;
                    Rect r = new Rect(targetScreenPosition, Vector2.zero);
                    float size = (ACESettings.ComposerSettings.TargetSize
                    ...
                    if (size > 0)
                    {
                        Vector4 overlayOpacityScalar
                            = new Vector4(1f, 1f, 1f, ACESettings.ComposerSettings.OverlayOpacity);
        ...
        private static void DrawGroupComposerGizmos(FramingTransposer target, GizmoType selectionType)
        {
            ...
            if (target.AbstractFollowTargetGroup != null
                && target.m_GroupFramingMode != FramingTransposer.FramingMode.None)
            {
