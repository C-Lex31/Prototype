...
namespace Ace.Editor
{
    ...
    public class VirtualCameraBaseEditor<T>
        : BaseEditor<T> where T : VirtualCamBase
    {
            ...
            if (sExtensionTypes == null)
            {
                ...
                var allExtensions
                    = ReflectionHelpers.GetTypesInAllDependentAssemblies(
                            (Type t) => typeof(Extension).IsAssignableFrom(t) && !t.IsAbstract);
        ...
        {
            if (ACEBrain.SoloCamera == (ICamera)Target)
            {
                ACEBrain.SoloCamera = null;
        ...
        protected void DrawExtensionsWidgetInInspector()
        {
            if (!IsPropertyExcluded("Extensions"))
            {
                ...
                if (selection > 0)
                {
                    ...
                    {
                        var targetGO = (targets[i] as VirtualCamBase).gameObject;
        ...
        protected void DrawCameraStatusInInspector()
        {
            ...
            bool isSolo = (ACEBrain.SoloCamera == (ICamera)Target);
            if (isSolo)
                GUI.color = ACEBrain.GetSoloGUIColor();

            bool isLive = ACECore.Instance.IsLive(Target);
            GUI.enabled = isLive;
            ...
            UpdateTracker.UpdateClock updateMode = ACECore.Instance.GetVcamUpdateStatus(Target);
            ...
            {
                isSolo = !isSolo;
                ACEBrain.SoloCamera = isSolo ? Target : null;
                InspectorUtility.RepaintGameView();
            }
            GUI.color = color;
        ...
        {
            ACESettings.ACECoreSettings.ShowInGameGuides
