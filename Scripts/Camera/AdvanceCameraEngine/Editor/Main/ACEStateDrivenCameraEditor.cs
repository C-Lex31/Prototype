...
{
    [CustomEditor(typeof(ACEStateDrivenCamera))]
    internal sealed class ACEStateDrivenCameraEditor
        :VirtualCameraBaseEditor<ACEStateDrivenCamera>
    {
        public EmbeddeAssetEditor<ACEBlenderConfig> m_BlendsEditor;
        ...
        {
            base.OnEnable();
            m_BlendsEditor = new EmbeddeAssetEditor<ACEBlenderConfig>(
                    FieldPath(x => x.m_CustomBlends), this);
            m_BlendsEditor.OnChanged = (ACEBlenderConfig b) =>
                {
                    InspectorUtility.RepaintGameView();
                };
            m_BlendsEditor.OnCreateEditor = (UnityEditor.Editor ed) =>
                {
                    ACEBlenderConfigEditor editor = ed as ACEBlenderConfigEditor;
                    if (editor != null)
        ...
        private void UpdateTargetStates()
        {
            ...
            List<ACEStateDrivenCamera.ParentHash> parents
                = new List<ACEStateDrivenCamera.ParentHash>();
            foreach (var i in collector.mStateParentLookup)
                parents.Add(new ACEStateDrivenCamera.ParentHash(i.Key, i.Value));
                ...
                for (int i = 0; i < states.Length; i++)
                {
                    ...
                    if (clips.Count > 1)
                    {
                        ...
                        foreach (AnimationClip c in clips)
                            AddState(
                                ACEStateDrivenCamera.CreateFakeHash(hash, c),
        ...
        private Dictionary<VirtualCamBase, int> mCameraIndexLookup;
        private void UpdateCameraCandidates()
        {
            List<string> vcams = new List<string>();
            mCameraIndexLookup = new Dictionary<VirtualCamBase, int>();
            vcams.Add("(none)");
            VirtualCamBase[] children = Target.ChildCameras;
        ...
        private int GetCameraIndex(Object obj)
        {
            if (obj == null || mCameraIndexLookup == null)
                return 0;
            VirtualCamBase vcam = obj as VirtualCamBase;
            if (vcam == null)
                return 0;
        ...
        void SetupInstructionList()
        {
            ...
            ACEStateDrivenCamera.Instruction def = new ACEStateDrivenCamera.Instruction();
            ...
            mInstructionList.onAddDropdownCallback = (Rect buttonRect, UnityEditorInternal.ReorderableList l) =>
                {
                    ...
                    menu.AddItem(new GUIContent("All Unhandled States"),
                        false, (object data) =>
                    {
                        ACEStateDrivenCamera target = Target;
            ...
            mChildList.onChangedCallback = (UnityEditorInternal.ReorderableList l) =>
                {
                    ...
                    VirtualCamBase vcam = (o != null)
                        ? (o as VirtualCamBase) : null;
                    if (vcam != null)
            ...
            mChildList.onAddCallback = (UnityEditorInternal.ReorderableList l) =>
                {
                    ...
                    var vcam = ACEMenu.CreateDefaultVirtualCamera();
