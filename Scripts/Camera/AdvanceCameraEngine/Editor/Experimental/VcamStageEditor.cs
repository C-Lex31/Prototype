...
namespace Ace.Editor
{
    internal class VcamStageEditor
    {
        ...
        ACECore.Stage mStage;
        ACEComponentBase mComponent;
        ...
        public VcamStageEditor(ACECore.Stage stage, GameObject target)
        {
            mStage = stage;
            Target = target;
        }
        ...
        public void OnInspectorGUI(ACEComponentBase component)
        {
        ...
        private int GetPopupIndexForComponent(ACEComponentBase c)
        {
            if (c != null)
            {
        ...
        public delegate void DestroyComponentDelegate(ACEComponentBase component);
    ...
    internal class VcamPipelineStageSubeditorSet
    {
        ...
        public void CreateSubeditors(UnityEditor.Editor parentEditor)
        {
            ...
            m_subeditors = new VcamStageEditor[(int)ACECore.Stage.Finalize];
            ACENewVirtualCamera owner = mParentEditor == null
                ? null : mParentEditor.target as ACENewVirtualCamera;
            if (owner == null)
                return;
            for (ACECore.Stage stage = ACECore.Stage.Body;
                stage < ACECore.Stage.Finalize; ++stage)
            {
                var ed = new VcamStageEditor(stage, owner.gameObject);
                m_subeditors[(int)stage] = ed;
                ed.SetComponent = (type)
                    => {
                        var vcam = mParentEditor.target as ACENewVirtualCamera;
                        if (vcam != null)
                        {
