...
namespace Ace
{
    ...
    public class ACECollider : Extension
    {
        ...
        public bool IsTargetObscured(ICamera vcam)
        {
        ...
        public bool CameraWasDisplaced(ICamera vcam)
        {
        ...
        /// <returns>True if the virtual camera has been displaced due to collision or
        /// target obstruction</returns>
        public float GetCameraDisplacementDistance(ICamera vcam)
        {
        ...
        float GetPushBackDistance(Ray ray, Plane startPlane, float targetDistance, Vector3 lookAtPos)
        {
            ...
            float angle = Mathf.Abs(VectorExtensions.Angle(startPlane.normal, ray.direction) - 90);
        ...
        private bool IsTargetOffscreen(CameraState state)
        {
            if (state.HasLookAt)
            {
                ...
                else
                {
                    ...
                    float angle = VectorExtensions.Angle(dir.ProjectOntoPlane(Vector3.right), Vector3.forward);
                    if (angle > fov)
                        return true;

                    fov = Mathf.Rad2Deg * Mathf.Atan(Mathf.Tan(fov * Mathf.Deg2Rad) * state.Lens.Aspect);
                    angle = VectorExtensions.Angle(dir.ProjectOntoPlane(Vector3.up), Vector3.forward);
                    if (angle > fov)
                        return true;
                }
            }
