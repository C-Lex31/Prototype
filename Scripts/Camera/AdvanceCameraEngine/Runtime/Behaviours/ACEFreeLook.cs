...
#endif
    [ExcludeFromPreset]
    [AddComponentMenu("Ace/Ace Free Look")]
    [HelpURL(Documentation.BaseURL + "manual/CinemachineFreeLook.html")]
    public class ACEFreeLook : VirtualCamBase
    {
        ...
        public OrbitalTransposer.Heading m_Heading
            = new OrbitalTransposer.Heading(
                OrbitalTransposer.Heading.HeadingDefinition.TargetForward, 4, 0);
        ...
        public OrbitalTransposer.BindingMode m_BindingMode
            = OrbitalTransposer.BindingMode.SimpleFollowWithWorldUp;
        ...
        protected override void OnValidate()
        {
            ...
            if (m_LegacyHeadingBias != float.MaxValue)
            {
                ...
                if (m_RecenterToTargetHeading.LegacyUpgrade(ref heading, ref m_Heading.m_VelocityFilterStrength))
                    m_Heading.m_Definition = (OrbitalTransposer.Heading.HeadingDefinition)heading;
            ...
            for (int i = 0; m_Rigs != null && i < 3 && i < m_Rigs.Length; ++i)
                if (m_Rigs[i] != null)
                    ACEVirtualCamera.SetFlagsForHiddenChild(m_Rigs[i].gameObject);
#endif
        }
        ...
        public ACEVirtualCamera GetRig(int i)
        {
            UpdateRigCache();
        ...
        public override bool IsLiveChild(ICamera vcam, bool dominantChildOnly = false)
        {
            ...
            {
                if (vcam == (ICamera)m_Rigs[0])
                    return y > 0.666f;
                if (vcam == (ICamera)m_Rigs[2])
                    return y < 0.333;
                if (vcam == (ICamera)m_Rigs[1])
            ...
            if (vcam == (ICamera)m_Rigs[1])
                return true;
            if (y < 0.5f)
                return vcam == (ICamera)m_Rigs[2];
            return vcam == (ICamera)m_Rigs[0];
        }
        ...
        public override void ForceCameraPosition(Vector3 pos, Quaternion rot)
        {
            ...
            UpdateRigCache();
            if (m_BindingMode != Transposer.BindingMode.SimpleFollowWithWorldUp)
        ...
        override public void InternalUpdateCameraState(Vector3 worldUp, float deltaTime)
        {
            ...
            InvokePostPipelineStageCallback(this, ACECore.Stage.Finalize, ref m_State, deltaTime);
            PreviousStateIsValid = true;

            // Set up for next frame
            bool activeCam = PreviousStateIsValid && ACECore.Instance.IsLive(this);
        ...
        public override void OnTransitionFromCamera(
            ICamera fromCam, Vector3 worldUp, float deltaTime)
        {
...
//            m_YAxis.m_Recentering.CancelRecentering();
            if (fromCam != null && m_Transitions.m_InheritPosition)
            {
                ...
                if (fromCam is ACEFreeLook)
                {
                    var flFrom = (fromCam as ACEFreeLook);
        ...
        private ACEVirtualCamera[] m_Rigs = new ACEVirtualCamera[3];

        void InvalidateRigCache() { mOrbitals = null; }
        OrbitalTransposer[] mOrbitals = null;
        ACEBlend mBlendA;
        ACEBlend mBlendB;
        ...
        /// The override must do exactly the same thing as the CreatePipeline method in this class.
        /// </summary>
        public delegate ACEVirtualCamera CreateRigDelegate(
        ...
        {
            ACEVirtualCamera[] oldRigs = new ACEVirtualCamera[RigNames.Length];
            for (int i = 0; i < RigNames.Length; ++i)
            {
                foreach (Transform child in transform)
                    if (child.gameObject.name == RigNames[i])
                        oldRigs[i] = child.GetComponent<ACEVirtualCamera>();
            }
        ...
        private ACEVirtualCamera[] CreateRigs(ACEVirtualCamera[] copyFrom)
        {
            // Invalidate the cache
            mOrbitals = null;
            float[] softCenterDefaultsV = new float[] { 0.5f, 0.55f, 0.6f };
            ACEVirtualCamera[] newRigs = new ACEVirtualCamera[3];
            for (int i = 0; i < RigNames.Length; ++i)
            {
                ACEVirtualCamera src = null;
                ...
                else
                {
                    ...
                    newRigs[i] = go.AddComponent<ACEVirtualCamera>();
                    go = newRigs[i].GetComponentOwner().gameObject;
                    go.AddComponent<OrbitalTransposer>();
                    go.AddComponent<Composer>();
                }

                // Set up the defaults
                newRigs[i].InvalidateComponentPipeline();
                OrbitalTransposer orbital = newRigs[i].GetCinemachineComponent<OrbitalTransposer>();
                if (orbital == null)
                    orbital = newRigs[i].AddCinemachineComponent<OrbitalTransposer>(); // should not happen
                if (src == null)
                {
                    // Only set defaults if not copying
                    orbital.m_YawDamping = 0;
                    Composer composer = newRigs[i].GetCinemachineComponent<Composer>();
                    if (composer != null)
                    {
        ...
        private void UpdateRigCache()
        {
            ...
            foreach (var rig in m_Rigs)
            {
                ...
                rig.m_LockStageInInspector = new ACECore.Stage[] { ACECore.Stage.Body };
            }
#endif

            // Create the blend objects
            mBlendA = new ACEBlend(m_Rigs[1], m_Rigs[0], AnimationCurve.Linear(0, 0, 1, 1), 1, 0);
            mBlendB = new ACEBlend(m_Rigs[2], m_Rigs[1], AnimationCurve.Linear(0, 0, 1, 1), 1, 0);
        }

        private int LocateExistingRigs(string[] rigNames, bool forceOrbital)
        {
            CachedXAxisHeading = 0;
            mOrbitals = new OrbitalTransposer[rigNames.Length];
            m_Rigs = new ACEVirtualCamera[rigNames.Length];
            int rigsFound = 0;
            foreach (Transform child in transform)
            {
                ACEVirtualCamera vcam = child.GetComponent<ACEVirtualCamera>();
                if (vcam != null)
                {
                    ...
                    for (int i = 0; i < rigNames.Length; ++i)
                    {
                        if (mOrbitals[i] == null && go.name == rigNames[i])
                        {
                            ...
                            mOrbitals[i] = vcam.GetCinemachineComponent<OrbitalTransposer>();
                            if (mOrbitals[i] == null && forceOrbital)
                                mOrbitals[i] = vcam.AddCinemachineComponent<OrbitalTransposer>();
                            if (mOrbitals[i] != null)
                            {
        ...
        float UpdateXAxisHeading(OrbitalTransposer orbital, float deltaTime, Vector3 up)
        {
            if (this == null)   
                return 0; // deleted
        ...
        private float GetYAxisValue()
        {
            ...
            return (range > VectorExtensions.Epsilon) ? m_YAxis.Value / range : 0.5f;
        }
