...
namespace Ace.Utility
{
    public class PositionPredictor
    {
        ...
        public void AddPosition(Vector3 pos, float deltaTime, float lookaheadTime)
        {
            if (deltaTime < 0)
                Reset();
            if (m_HavePos && deltaTime > VectorExtensions.Epsilon)
            {
    ...
    {
        const float Epsilon = VectorExtensions.Epsilon;
    ...
    public class HeadingTracker
    {
        ...
        void PopBottom()
        {
            if (mCount > 0)
            {
                float time = ACECore.CurrentTime;
                ...
                if (mWeightSum <= VectorExtensions.Epsilon || mCount == 0)
                    ClearHistory();
            }
        }
        ...
        public void DecayHistory()
        {
            ...
            mWeightSum *= decay;
            mWeightTime = time;
            if (mWeightSum < VectorExtensions.Epsilon)
                ClearHistory();
            else
