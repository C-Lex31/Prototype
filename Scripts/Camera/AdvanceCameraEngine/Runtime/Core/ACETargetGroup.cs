...
namespace Ace
{
    ...
    /// </summary>
    public interface IACETargetGroup
    {
        /// <summary>
    ...
    [AddComponentMenu("Ace/Ace Target Group")]
    [SaveDuringPlay]
    ...
    public class ACETargetGroup : MonoBehaviour, IACETargetGroup
    {
        ...
        public bool IsEmpty
        {
            get
            {
                for (int i = 0; i < m_Targets.Length; ++i)
                    if (m_Targets[i].target != null && m_Targets[i].weight > VectorExtensions.Epsilon)
                        return false;
                return true;
            }
        }
            ...
            Target t, Vector3 avgPos, float maxWeight)
        {
            ...
            if (t.target != null)
            {
                ...
                if (maxWeight > VectorExtensions.Epsilon && w < maxWeight)
                    w /= maxWeight;
                else
                    w = 1;
            }
        ...
        Vector3 CalculateAveragePosition(out float maxWeight)
        {
            ...
            if (weight > VectorExtensions.Epsilon)
                pos /= weight;
            else
        ...
        {
            if (m_MaxWeight <= VectorExtensions.Epsilon)
            {
        ...
        Bounds CalculateBoundingBox(Vector3 avgPos, float maxWeight)
        {
            ...
            if (maxWeight > VectorExtensions.Epsilon)
            {
                for (int i = 0; i < m_Targets.Length; ++i)
                {
                    if (m_Targets[i].target != null)
            ...
            Matrix4x4 observer, out Vector2 minAngles, out Vector2 maxAngles, out Vector2 zRange)
        {
            ...
            for (int i = 0; i < m_Targets.Length; ++i)
            {
                ...
                if (p.z < VectorExtensions.Epsilon)
                    continue; // behind us
