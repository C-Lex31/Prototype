...
namespace Ace
{
    ...
    ///
    /// A VirtualCamBase exposes a Priority property.  When the behaviour is
    /// enabled in the game, the Virtual Camera is automatically placed in a queue
    /// maintained by the static ACECore singleton.
    ...
    /// </summary>
    [SaveDuringPlay]
    public abstract class VirtualCamBase : MonoBehaviour, ICamera
    {
        ...
        [HideInInspector, SerializeField, NoSaveDuringPlay]
        public ACECore.Stage[] m_LockStageInInspector;

        /// <summary>Version that was last streamed, for upgrading legacy</summary>
        public int ValidatingStreamVersion
        {
            get { return m_OnValidateCalled ? m_ValidatingStreamVersion : ACECore.kStreamingVersion; }
        ...
        {
            if (mExtensions == null)
                mExtensions = new List<Extension>();
            else
        ...
        public virtual void RemoveExtension(Extension extension)
        {
            if (mExtensions != null)
                mExtensions.Remove(extension);
        }
        ...
        protected void InvokePostPipelineStageCallback(
            VirtualCamBase vcam, ACECore.Stage stage,
            ref CameraState newState, float deltaTime)
        {
            ...
            VirtualCamBase parent = ParentCamera as VirtualCamBase;
            if (parent != null)
        ...
        protected void InvokePrePipelineMutateCameraStateCallback(
            VirtualCamBase vcam, ref CameraState newState, float deltaTime)
        {
            if (mExtensions != null)
            {
                for (int i = 0; i < mExtensions.Count; ++i)
        ...
        /// in the ACECore's queue of eligible shots.</summary>
        ...
        public virtual bool IsLiveChild(ICamera vcam, bool dominantChildOnly = false) { return false; }
        ...
        /// use ACECore.UpdateVirtualCamera(ICamera, Vector3, float), which
        ...
        {
            ACECore.Instance.UpdateVirtualCamera(this, worldUp, deltaTime);
        }

        /// <summary>Internal use only.  Do not call this method.
        /// Called by ACECore at designated update time
        ...
        public struct TransitionParams
        {
            ...
            public ACEBrain.VcamActivatedEvent m_OnCameraLive;
        }
        ...
        {
            ACECore.Instance.CameraDestroyed(this);
        }

        /// <summary>Base class implementation makes sure the priority queue remains up-to-date.</summary>
        protected virtual void OnTransformParentChanged()
        {
            ACECore.Instance.CameraDisabled(this);
            ACECore.Instance.CameraEnabled(this);
            UpdateSlaveStatus();
        ...
        internal void EnsureStarted()
        {
            ...
            {
                m_WasStarted = true;
                var extensions = GetComponentsInChildren<Extension>();
        ...
        protected virtual void OnValidate()
        {
            ...
            m_StreamingVersion = ACECore.kStreamingVersion;
        }
        ...
        protected virtual void OnEnable()
        {
            ...
            if (!ACECore.Instance.IsLive(this))
                PreviousStateIsValid = false;
            ACECore.Instance.CameraEnabled(this);
            // Sanity check - if another vcam component is enabled, shut down
            var vcamComponents = GetComponents<VirtualCamBase>();
        ...
        private VirtualCamBase m_parentVcam = null;
        ...
        {
            ACECore.Instance.RemoveActiveCamera(this);
            if (m_parentVcam == null && isActiveAndEnabled)
                ACECore.Instance.AddActiveCamera(this);
        ...
        protected CameraState PullStateFromVirtualCamera(Vector3 worldUp, ref LensSettings lens)
        {
            ...
            ACEBrain brain = ACECore.Instance.FindPotentialTargetBrain(this);
            if (brain != null)
