...
namespace Ace
{
    ...
    public interface ICamera
    {
        ...
        bool IsLiveChild(ICamera vcam, bool dominantChildOnly = false);

        /// <summary>
        /// Update the camera's state.
        /// The implementation must guarantee against multiple calls per frame, and should
        /// use ACECore.UpdateVirtualCamera(ICinemachineCamera, Vector3, mfloat), which
        ...
        /// ACECore.UpdateVirtualCamera(ICinemachineCamera, Vector3, float), which
        /// has protection against multiple calls per frame.
        /// </summary>
        /// <param name="worldUp">Default world Up, set by the CinemachineBrain</param>
        /// <param name="deltaTime">Delta time for time-based effects (ignore if less than 0)</param>
        ...
        /// <param name="worldUp">Default world Up, set by the CinemachineBrain</param>
        /// <param name="deltaTime">Delta time for time-based effects (ignore if less than 0)</param>
        void OnTransitionFromCamera(ICamera fromCam, Vector3 worldUp, float deltaTime);
