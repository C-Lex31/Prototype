...
namespace Ace
{
    ...
    [SaveDuringPlay]
    public class Composer : ACEComponentBase
    {
        ...
        public override ACECore.Stage Stage { get { return ACECore.Stage.Aim; } }
        ...
        struct FovCache
        {
            ...
            private Rect ScreenToFOV(Rect rScreen, float fov, float fovH, float aspect)
            {
                ...
                float angle = VectorExtensions.SignedAngle(Vector3.forward, p, Vector3.left);
                r.yMin = ((fov / 2) + angle) / fov;

                p = persp.MultiplyPoint(new Vector3(0, (r.yMax * 2f) - 1f, 0.5f)); p.z = -p.z;
                angle = VectorExtensions.SignedAngle(Vector3.forward, p, Vector3.left);
                r.yMax = ((fov / 2) + angle) / fov;

                p = persp.MultiplyPoint(new Vector3((r.xMin * 2f) - 1f, 0, 0.5f));  p.z = -p.z;
                angle = VectorExtensions.SignedAngle(Vector3.forward, p, Vector3.up);
        ...
        {
            float angle = VectorExtensions.Angle(dir, up);
