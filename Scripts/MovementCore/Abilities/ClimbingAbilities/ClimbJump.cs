...
{
#if SCRIPTED_ANIMATOR


    [SerializeField] private string m_JumpToHang = "Air.Jump To Hang";
    [SerializeField]  private float jumpHangClipDuration = 0.1f;
    [SerializeField] private string m_JumpToLowerClimb = "Air.Jump To Lower";
    [SerializeField]  private float jumpLowerClipDuration = 0.1f;

    private Dictionary<string, float> statesDuration = new Dictionary<string, float>();
#endif
    private ClimbingAbility m_ClimbAbility;
    [SerializeField] private string m_JumpToHang = "TryGrabLedge";
    [SerializeField] private float jumpHangClipDuration = 0.1f;
    // private string m_JumpToHang = "TryGrabLedge";
    private Dictionary<string, float> statesDuration = new Dictionary<string, float>();
    // ------------------------------------------------------------------
    // Physics Parameters
    // ------------------------------------------------------------------
    ...
    protected void Awake()
    {
        ...
        m_ClimbAbility = GetComponent<ClimbingAbility>();
    }
    ...
    {
        if ((m_System.m_IsGrounded || m_System.GroundCheckDistance < 0.05f)
            && UsePredictionMath)
        {
            if (!Mathf.Approximately(m_System.InputManager.RelativeInput.magnitude, 0))
            {
                ...
                foreach (ThirdPersonAbstractClimbing climbing in climbingAttached)
                {
                    ...
                    {
                        SetLaunchParameters(data, (climbing is UpperStepClimb) ? ClimbJumpType.FromGroundToLower :
                ...
                /*                LadderAbility ladder = GetComponent<LadderAbility>();
                                if (ladder != null)
                                {
                                    ...
                                    {
                                        LaunchData data = GetLaunchData(ladder.GrabPosition, m_System.m_Collider.height, NormalJumpParameters, direction, m_MaxAngle, Vector3.up, ladder.LadderMask);
    ...
    public override void OnEnterAbility()
    {
        ...
        //m_AnimatorManager.PerformBoolEvent(m_JumpToHang,true) ;    
    ...
    {
        if (m_System.m_IsGrounded || m_ClimbAbility.RefHasFoundLedge)
        {
            m_AnimatorManager.PerformBoolEvent("LedgeGrab", true);
            return true;
        }
    ...
    private void SetLaunchParameters(LaunchData data, ClimbJumpType targetJumpType)
    {
        ...
        m_JumpTimeToTarget = data.timeToTarget;
        //  m_AnimatorManager.PerformBoolEvent(m_JumpToHang,true) ;                                                      // Get total time to reach the target
    ...
    private bool IsLaunchDataOnlyWay(LaunchData data, Vector3 direction)
    {
        if (data.foundSolution) // If found a solution
        {
            ...
            {
                Vector3 cp1 = data.target + Vector3.up * m_System.m_Collider.radius * 2.1f;
                Vector3 cp2 = data.target + Vector3.up * (m_System.m_Collider.height - m_System.m_Collider.radius * 2) + direction * m_System.m_Collider.radius;

                if (Physics.OverlapCapsule(cp1, cp2, m_System.m_Collider.radius * 2, Physics.AllLayers, QueryTriggerInteraction.Ignore).Length > 0)
                    return true;
            }
        }
        return false;
    ...
    private bool FoundLedgeToGrab(out List<Collider> ledges, Vector3 direction, float LinecastStartPoint, LayerMask climbableMask, Collider ignoreCollider)
    {
        ...
        if (m_System.GroundHitInfo.collider != null && m_System.m_IsGrounded)
        {
    ...
    private List<Vector3> GetTargetPoints(List<Collider> ledges, Vector3 launcOriginPoint, Vector3 targetDirection, Vector3 offset)
    {
        ...
        foreach (Collider coll in ledges)
        {
            ...
            if (coll.gameObject.layer == 20)
            {
                ...
                ladderClosestPoint.y = transform.position.y + m_System.m_Collider.height * 0.5f;
                ladderClosestPoint.y = Mathf.Clamp(ladderClosestPoint.y, minVertical + m_System.m_Collider.height, verticalPoint);
    ...
    private void LaunchCharacter(Vector3 m_JumpDirectionVector, Vector3 GrabPosition, float LinecastStart, Collider ignoreCollider = null)
    {
        ...
        switch (m_JumpType)
        {
            ...
            case ClimbJumpType.Right:
                parameter = HopRightParameters;
                offset = transform.right * m_System.m_Collider.radius;   // Add a small offset to the right to avoid grab on corners
                break;
            case ClimbJumpType.Left:
                parameter = HopLeftParameters;
                offset = -transform.right * m_System.m_Collider.radius;   // Add a small offset to the left to avoid grab on corners
                break;
            case ClimbJumpType.Back:
                parameter = JumpBackParameters;
                break;
