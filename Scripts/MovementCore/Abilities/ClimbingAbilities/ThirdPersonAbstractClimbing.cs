...
public abstract class ThirdPersonAbstractClimbing : ThirdPersonAbility
{
    ...
    [Tooltip("How many iteration engines should do after finding a ledge")] [SerializeField] protected int m_Iterations = 10;
    ...
    {
        m_System.m_Collider.enabled = true;
    ...
    protected bool HasFoundLedge(out RaycastHit capsuleHit, bool m_IsAlreadyClimbing = true)
    {
        ...
        if (!m_IsAlreadyClimbing || m_FinishOnAnimationEnd)
        {
            ...
            Collider[] overlappedLedges = Physics.OverlapCapsule(overlapPoint1, overlapPoint2, m_System.m_Collider.radius * 2, m_ClimbableMask, QueryTriggerInteraction.Collide);
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            else
            {
                ...
                // Check side length
                // Check if its corner
                Vector3 rightOrigin = hits[i].point + hitReference.right * (m_System.m_Collider.radius * 1.5f);
                Vector3 lefttOrigin = hits[i].point - hitReference.right * (m_System.m_Collider.radius * 1.5f);
                float rightAngle = 0;
                float leftAngle = 0;
                ...
                float delta = (m_System.m_Collider.radius * 3) / (m_Iterations * 2);
                Vector3 left = Vector3.zero;
                Vector3 right = Vector3.zero;
                for (int j = 0; j < m_Iterations * 2; j++)
                {
                    Vector3 checkpoint = hits[i].point + hitReference.transform.right * (-m_System.m_Collider.radius * 1.5f + delta * j);
                ...
                if (Vector3.Distance(left, right) >= m_System.m_Collider.radius * 1.5f)
                {
    ...
    protected bool FreeAboveLedge()
    {
        ...
        Vector3 capsulePoint2 = capsulePoint1 + Vector3.up * (m_System.m_Collider.height * 0.5f - m_CastCapsuleRadius * 2);

        Vector3 groundCheckStart = topHit.point + Vector3.up + transform.forward * m_System.m_Collider.radius * 2f;
